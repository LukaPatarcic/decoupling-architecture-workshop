...
describe("Test 'tasks' service", () => {
	describe("Test actions", () => {
		...
		describe("Test 'tasks.increaseQuantity'", () => {
			test("should call the adapter updateById method & transform result", async () => {
				...
				const res = await broker.call("tasks.increaseQuantity", {
					id: "123",
					value: 10,
				});
				expect(res).toEqual({
		...
		describe("Test 'tasks.decreaseQuantity'", () => {
			test("should call the adapter updateById method & transform result", async () => {
				service.adapter.updateById.mockClear();
				service.transformDocuments.mockClear();
				service.entityChanged.mockClear();

				const res = await broker.call("tasks.decreaseQuantity", {
					id: "123",
					value: 10,
				});
				expect(res).toEqual({
			...
			test("should throw error if params is not valid", async () => {
				...
				expect.assertions(2);
				try {
					await broker.call("tasks.decreaseQuantity", {
						id: "123",
						value: -5,
					});
				} catch (err) {
